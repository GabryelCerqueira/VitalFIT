...
export async function readDatabase() {
    ...
    if (!db.posts)
        db.posts = defaultDb.posts;
    if (!db.plans || db.plans.length === 0)
        db.plans = defaultPlans;
    if (!db.alimentos || db.alimentos.length === 0)
        db.alimentos = defaultAlimentos;
    if (!db.dietas || db.dietas.length === 0)
        db.dietas = defaultDietas;
    return db;
}
