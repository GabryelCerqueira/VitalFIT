...
import type { Dieta24h, Refeicao } from '../../types/domain.js';
import type { AuthedRequest } from '../../middlewares/auth.middleware.js';

export async function listFoods(_req: Request, res: Response): Promise<void> {
  const db = await readDatabase();
  res.json({ alimentos: db.alimentos || [] });
}

export async function listDiets(_req: Request, res: Response): Promise<void> {
  const db = await readDatabase();
  res.json({ dietas: db.dietas || [] });
}

export async function createDiet(req: Request, res: Response): Promise<void> {
  ...
  const novaDieta: Dieta24h = {
    ...
    refeicoes: (refeicoes as Array<Partial<Refeicao>>).map((r) => ({
...
export async function cloneDiet(req: Request, res: Response): Promise<void> {
  const userId = (req as AuthedRequest).userId;
  const db = await readDatabase();
  const user = db.users.find((u) => u.id === userId);
...
export async function deleteDiet(req: Request, res: Response): Promise<void> {
  const userId = (req as AuthedRequest).userId;
  const db = await readDatabase();
  const user = db.users.find((u) => u.id === userId);
